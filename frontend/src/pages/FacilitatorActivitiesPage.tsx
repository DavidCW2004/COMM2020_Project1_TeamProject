...
function normalizePhases(raw: any): Phase[] {
    ...
    return raw.map((phase) => {
        ...
        return {
            ...
            assessment_criteria: Array.isArray(phase?.assessment_criteria) ? phase.assessment_criteria : [],
        };
    });
}
...
export default function ActivityManagementView() {
    ...
    function addPhase() {
        ...
        setPhases((prev) => [
            ...prev,
            {
                ...
                assessment_criteria: [...globalCriteria],
            },
        ]);
    }
    ...
    function addCriterion() {
        ...
        setPhases((prev) =>
            prev.map((p) => ({
                ...p,
                assessment_criteria: [...(p.assessment_criteria ?? []), trimmed],
            }))
        );
    }
                ...
                <div className="rounded-lg border border-border bg-background/80 backdrop-blur p-6 shadow-sm">
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div className="min-w-0">
                            <h1 className="text-2xl font-semibold tracking-tight">Facilitator activity tools</h1>
                            <p className="mt-1 text-sm text-muted-foreground">
                                Create and edit activities, and review agent support settings.
                            </p>
                        </div>
                        <Button variant="secondary" onClick={() => navigate(dashboardPath)} disabled={busy}>
                            Back to dashboard
                        </Button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    ...
                    <div className="rounded-lg border border-border bg-background shadow-sm p-5 space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-lg font-semibold">{editingActivityId ? "Edit activity" : "Create activity"}</h2>
                            {editingActivityId ? (
                            ...
                            {phases.map((p, idx) => (
                                <div key={idx} className="rounded-lg border border-border p-3 space-y-2">
                                 ...
                                 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                                    ...
                                    <Button variant="secondary" onClick={() => removePhase(idx)} disabled={busy || phases.length <= 1}>
                                        Remove phase
                                    </Button>
                                </div>
                            ))}
                        ...
                        <div className="space-y-3">
                            ...
                            <p className="text-sm text-muted-foreground">Configure how support agents behave for this activity.</p>
                            ...
                            <div className="rounded-lg border border-border p-3 space-y-3">
                                <div className="flex items-center justify-between gap-4">
                                    <div>
                                        ...
                                        <div className="text-xs text-muted-foreground">Detects uneven participation.</div>
                                    </div>
                                    ...
                                    <div className="mt-1">
                                        <Input
                                            ...
                                            onChange={(e) =>
                                                setAgentSettings((prev) => ({
                                                    ...prev,
                                                    equity: { ...prev.equity, participation_gap: Number(e.target.value) || 1 },
                                                }))
                                            }
                            ...
                            <div className="rounded-lg border border-border p-3 space-y-3">
                                <div className="flex items-center justify-between gap-4">
                                    <div>
                                        ...
                                        <div className="text-xs text-muted-foreground">Nudges inactive participants.</div>
                                    </div>
                                    <label className="flex items-center gap-2 text-sm">
                                        <input
                                            type="checkbox"
                                    ...
                                    <div className="mt-1">
                                        <Input
                                            ...
                                            onChange={(e) =>
                                                setAgentSettings((prev) => ({
                                                    ...prev,
                                                    inactivity: { ...prev.inactivity, idle_seconds: Number(e.target.value) || 10 },
                                                }))
                                            }
                            ...
                            <div className="rounded-lg border border-border p-3 space-y-3">
                                <div className="flex items-center justify-between gap-4">
                                    <div>
                                        ...
                                        <div className="text-xs text-muted-foreground">Nudges users to support claims.</div>
                                    </div>
                                    <label className="flex items-center gap-2 text-sm">
                                        <input
                                            type="checkbox"
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) =>
                                                setAgentSettings((prev) => ({
                                                    ...
                                                    evidence: {
                                                        ...prev.evidence,
                                                        unsupported_claims_before_nudge: Number(e.target.value) || 1,
                                                    },
                                                }))
                                            }
                            ...
                            <div className="flex flex-wrap gap-2">
                                {globalCriteria.map((criterion, idx) => (
                                    <button
                                        key={`${criterion}-${idx}`}
                                        type="button"
                                        className="rounded-full border border-border bg-muted/30 px-3 py-1 text-sm hover:bg-muted/50 transition
                               focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                                ...
                                <div className="space-y-2">
                                    {activities.map((activity) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex gap-2">
                                                <Button variant="secondary" onClick={() => beginEditActivity(activity)} disabled={busy}>
                                                    Edit
                                                </Button>
                                            </div>
                                        </div>
